...
export function Hero() {
  ...
  return (
    <section
      ref={heroRef}
      className="flex min-h-130 w-full max-w-full flex-col bg-cream px-[clamp(1.125rem,5.5vw,4.5rem)] pt-7 md:h-screen md:pt-9"
    >
      <IntroScrollProvider sectionRef={heroRef}>
        <div className="mx-auto flex w-fit min-w-0 max-w-[1305px] flex-1 flex-col">
          ...
          <HeroNavBehindMotion
            heroRef={heroRef}
            lag={0}
            className="mt-40! flex w-full min-w-0 flex-col items-center gap-3 text-center md:mt-9 md:flex-row md:items-baseline md:justify-between md:gap-4 md:text-left"
          >
            <HeroItemMotion item="name">
              <p className="w-full max-w-full font-heading text-[clamp(1.5rem,7.86vw,113.1px)] font-normal leading-[1.06] tracking-[-0.02em] text-navy">
                Babajide Olatunji
              </p>
            </HeroItemMotion>
          ...
          <div className="relative z-10 mt-2 flex w-full min-w-0 flex-1 items-start justify-center bg-cream pb-6 md:pb-10">
            <HeroTitleMotion preset="orbit" beat={2} delay={0.1}>
              <h1 className="relative w-full bg-cream text-center font-display text-[clamp(5rem,34.6vw,497.89px)] font-normal uppercase leading-none tracking-[-0.02em] text-orange">
                UMBERMAN
              </h1>
            </HeroTitleMotion>
          </div>
